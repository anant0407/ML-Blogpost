...
# Introduce an anomaly (outlier)
anomaly = np.array([[0, 0]])

# Combine the original data with the anomaly
data_with_anomaly = np.vstack([data, anomaly])

# Calculate Z-scores for each feature
z_scores = zscore(data_with_anomaly, axis=0)
...
# Visualize the data with anomaly
plt.scatter(data_with_anomaly[:, 0], data_with_anomaly[:, 1], s=30, cmap='viridis')
plt.scatter(data_with_anomaly[anomalies[0], 0], data_with_anomaly[anomalies[0], 1], color='red', s=100, label='Anomalies')
plt.title('Data with Anomaly for Z-score Anomaly Detection')
plt.legend()
plt.show()
```
